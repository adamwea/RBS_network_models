...
def generate_neuron_params(cfg, neuron_id, cellType, x=None, y=None, z=None, xRange=None, yRange=None, zRange=None):
    ...
    cell_params['pointps'] = [{'loc': [x if x is not None else np.random.uniform(xRange[0], xRange[1]),
...
def create_network(cfg, numExcitatory, numInhibitory, cell_positions=None):
    ...
    for i in range(numExcitatory):
        ...
        netParams.cellParams[f'Exc_{i}'] = generate_neuron_params(cfg, i, 'E', *position)
    ...
    for i in range(numInhibitory):
        ...
        netParams.cellParams[f'Inh_{i}'] = generate_neuron_params(cfg, i + numExcitatory, 'I', *position)
...
def main(cell_positions = None):
    ...
    # Number of neurons
    numExcitatory, numInhibitory = 100, 43
