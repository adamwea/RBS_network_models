...
DEFAULT_NETPARAMSFILE = None
DEFAULT_INIT_SCRIPT = None
...
DEFAULT_FITNESS_TARGET_SCRIPT = None
...
DEFAULT_RECALCULATE_FITNESS = False
...
def parse_arguments():
    ...
    parser.add_argument('-diln', '--debugging_in_login_node', action='store_true', help='Debugging in Login Node')
    parser.add_argument('-dicn', '--debugging_in_compute_node', action='store_true', help='Debugging in Compute Node')
    parser.add_argument('-oicn', '--optimizing_in_compute_node', action='store_true', help='Optimizing in Compute Node')
    parser.add_argument('-nps', '--nodes_per_simulation', type=int, help='Nodes per Simulation')
    parser.add_argument('-spn', '--sockets_per_simulation', type=int, help='Sockets per Simulation')
    parser.add_argument('-rrf', '--recalculate_fitness', action='store_true', help='Recalculate Fitness')
    parser.add_argument('-ty', '--type', type=str, help='Type')
...
def configure_global_user_vars(**kwargs):
    ...
    USER_vars = {
        'USER_coresPerNode': get_arg_value(getattr(args, 'coresPerNode', None), DEFAULT_CORES_PER_NODE),
        'USER_continue_run': get_arg_value(getattr(args, 'continue_run', None), False),
        'USER_pop_size': get_arg_value(getattr(args, 'pop_size', None), DEFAULT_POP_SIZE),
        'USER_max_generations': get_arg_value(getattr(args, 'max_generations', None), DEFAULT_MAX_GENERATIONS),
        'USER_seed_evol': get_arg_value(getattr(args, 'seed_evol', None), DEFAULT_USER_SEED_EVOL),
        'USER_method': get_arg_value(getattr(args, 'method', None), DEFAULT_METHOD),
        'USER_cfg_file': get_arg_value(getattr(args, 'cfg_file', None), DEFAULT_CFG_FILE),
        'USER_netParamsFile': get_arg_value(getattr(args, 'netParamsFile', None), DEFAULT_NETPARAMSFILE),
        'USER_init_script': get_arg_value(getattr(args, 'init_script', None), DEFAULT_INIT_SCRIPT),
        'USER_mpiCommand': get_arg_value(getattr(args, 'mpiCommand', None), ''),
        'USER_nrnCommand': get_arg_value(getattr(args, 'nrnCommand', None), DEFAULT_NRNCOMMAND),
        'USER_nodes': get_arg_value(getattr(args, 'nodes', None), DEFAULT_NODES),
        #'USER_cores_per_node': get_arg_value(getattr(args, 'cores_per_node', None), DEFAULT_CORES_PER_NODE),
        'USER_skip': get_arg_value(getattr(args, 'skip', None), DEFAULT_SKIP),
        'USER_num_elites': get_arg_value(getattr(args, 'num_elites', None), DEFAULT_NUM_ElITES),
        'USER_mutation_rate': get_arg_value(getattr(args, 'mutation_rate', None), DEFAULT_MUTATION_RATE),
        'USER_crossover': get_arg_value(getattr(args, 'crossover', None), DEFAULT_CROSSOVER_RATE),
        'USER_time_sleep': get_arg_value(getattr(args, 'time_sleep', None), DEFAULT_TIME_SLEEP),
        'USER_maxiter_wait': get_arg_value(getattr(args, 'maxiter_wait', None), DEFAULT_MAXITER_WAIT),
        'USER_plot_fitness': get_arg_value(getattr(args, 'plot_fitness', None), DEFAULT_PLOT_FITNESS),
        'USER_num_excite': get_arg_value(getattr(args, 'num_excite', None), DEFAULT_NUM_EXCITE),
        'USER_num_inhib': get_arg_value(getattr(args, 'num_inhib', None), DEFAULT_NUM_INHIB),
        'USER_seconds': get_arg_value(getattr(args, 'duration', None), DEFAULT_DURATION),
        'USER_runCfg_type': get_arg_value(getattr(args, 'type', None), DEFAULT_MPI_TYPE),
        'USER_run_label': get_arg_value(getattr(args, 'label', None), 'vscode'),
        'USER_mpis_per_batch': get_arg_value(getattr(args, 'tasks', None), 16),
        'USER_run_path': run_path,
        'USER_output_path': getattr(args, 'simulation_output_path', None),
        'USER_continue': get_arg_value(getattr(args, 'continue_run', None), False),
        'USER_overwrite': get_arg_value(getattr(args, 'overwrite', None), False),
        'USER_fitness_target_script': get_arg_value(getattr(args, 'fitness_target_script', None), DEFAULT_FITNESS_TARGET_SCRIPT),
        'USER_debug_in_login_node': get_arg_value(getattr(args, 'debugging_in_login_node', None), DEFAULT_DEBUG_IN_LOGIN_NODE),
        'USER_debug_in_compute_node': get_arg_value(getattr(args, 'debugging_in_compute_node', None), DEFAULT_DEBUG_IN_COMPUTE_NODE),
        'USER_optimize_in_compute_node': get_arg_value(getattr(args, 'optimizing_in_compute_node', None), DEFAULT_OPTIMIZE_IN_COMPUTE_NODE),
        'USER_nodes_per_simulation': get_arg_value(getattr(args, 'nodes_per_simulation', None), DEFAULT_NODES_PER_SIMULATION),
        'USER_sockets_per_node': get_arg_value(getattr(args, 'sockets_per_simulation', None), DEFAULT_SOCKKETS_PER_NODE),
        'USER_recalculate_fitness': get_arg_value(getattr(args, 'recalculate_fitness', None), DEFAULT_RECALCULATE_FITNESS)
    }
...
def save_config_to_file(USER_vars, **kwargs):
    run_path = USER_vars['USER_run_path']
    #temp_user_args_path = f'temp_user_args.py'
    #get path of __main__ script
    script_path = os.path.dirname(os.path.realpath(__file__))
    temp_user_args_path = f'{script_path}/temp_user_args.py'
