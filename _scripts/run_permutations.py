...
PROJECT_OUTPUT_DIR = '/global/homes/a/adammwea/pscratch/z_simulated_data/CDKL5-E6D_T2_C1_05212024/DIV21_FxHET/' # output directory for simulated data, corresponding to the project
...
kwargs = {
    ...
    # init
    'overwrite': True,  # if True, overwrite the latest sensitivity analysis run, if False, create a new run directory
    'origin_sim_path': ORIGIN,  # Original simulation data path, starting point for the sensitivity analysis
    #'sa_output_dir': SA_OUTPUT_DIR,  # output directory for the sensitivity analysis results, correspoinding to project
    'proj_output_dir': PROJECT_OUTPUT_DIR,  # output directory for the simulation data, if not specified, it will be derived from the simulation data path
    
    # permute cfg and netParams
    #'evol_params': params,  # evolution parameters for the simulation data, if not specified, it will be derived from the simulation data path
    'evol_path': EVOL_PATH,  # path to the evolution parameters for the simulation data, if not specified, it will be derived from the simulation data path
    ...
    'mpi': True,  # if True, run the simulations in parallel using MPI, if False, run the simulations using multiprocessing or sequentially (only if parallel is True)
    'slurm': True,  # if True, run the simulations using srun, if False, run the simulations using mpirun (only if mpi is True)
    ...
    'tasks_per_node': 4,  # number of tasks per node for the simulation, default to 1 task per node, can be adjusted based on the number of cores available
    
    # analysis
    #'conv_params': conv_params,  # conversion parameters for the simulation data, if not specified, it will be derived from the simulation data path
    #'mega_params': mega_params,  # mega parameters for the simulation data, if not specified, it will be derived from the simulation data path
    'conv_path': CONV_PATH,  # path to the conversion parameters for the simulation data, if not specified, it will be derived from the simulation data path
}
