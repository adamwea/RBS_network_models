...
from workspace.RBS_network_simulations.workspace.optimization_projects.CDKL5_DIV21_dep._2_batchrun_optimization.calculate_fitness_v3 import fitnessFunc
...
def analyze_simulations_parallel(simulation_run_paths, reference=False, reference_data=None, reference_raster=None, reference_bursting=None, max_workers=4, debug=False):
...
def generate_network_summary_slide_content(data_path, reference=False, reference_data=None, reference_raster=None, reference_bursting=None):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("conv_params_module", CONVOLUTION_PARAMS)
        conv_params_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(conv_params_module)

        # Access conv_params from the module
        conv_params = conv_params_module.conv_params
