...
def process_pdf_to_images(pdf_path, fitness_path):
    ...
    for page_num in range(len(doc)):
        ...
        images.append((cand_name, fitness_path, pdf_path, page_num + 1, image_stream))
    doc.close()
    return images
...
def add_slide_with_image(prs, image_stream, title, average_fitness, output_dir):
    ...
    # Save an image and PDF of the slide
    image_path = os.path.join(output_dir, f"{title}_summary_slide.png")
    pdf_path = os.path.join(output_dir, f"{title}_summary_slide.pdf")
    generate_image_and_pdf(image_stream, title, average_fitness, image_path, pdf_path)
    
    return pdf_path

def generate_image_and_pdf(image_stream, title, average_fitness, image_path, pdf_path):
...
# pdf_path = add_slide_with_image(prs, 'path_to_image.png', 'Candidate_1', 0.85, output_dir)
# print(f'Generated PDF for candidate at {pdf_path}')
...
def collect_network_summary_plots(simulation_run_paths, fitness_data, file_extension='png', max_workers=4):
    """
            ...
            for root, _, files in os.walk(simulation_run_path):
                for file in files:
                    if file.endswith(f'_summary_plot.{file_extension}'):
                        ...
                        # else:
                        #     pdf_slide_path = add_slide_with_image(prs, network_summary_path, cand_name, average_fitness)
    
    # Collect results from tasks and add slides sequentially
    for task in tasks:
        try:
            images = task.result()
            for cand_name, fitness_path, pdf_path, page_num, image_stream in images:                
                #fitness_path = os.path.join(root, cand_name + '_fitness.json')
                average_fitness = fitness_dict.get(fitness_path, "N/A")
                #pdf_list.append(pdf_path)
                output_dir = os.path.dirname(fitness_path)
                pdf_slide_path = add_slide_with_image(prs, image_stream, f"{cand_name}", average_fitness, output_dir)
    ...
    #save the pdfs as one pdf
    #pdf_path = os.path.join(progress_slides_path, 'CDKL5_DIV21_campaign_1.pdf')
    # with PdfPages(pdf_path) as pdf:
    #     for pdf_slide in pdf_list:
    #         pdf.savefig(pdf_slide)
    
    # #get paths in pdf_list and save as one pdf
    # pdf_paths = [pdf for pdf in pdf_list]
    # pdf_path = os.path.join(progress_slides_path, 'CDKL5_DIV21_campaign_1.pdf')
    # with PdfPages(pdf_path) as pdf:
    #     for pdf_slide in pdf_paths:
    #         pdf.savefig(pdf_slide)
...
if __name__ == "__main__":    
    ...
    if switch:
        ...
        collect_network_summary_plots(simulation_run_paths, fitness_data, file_extension='pdf', max_workers=number_physical_cpus)    
