...
from workspace.RBS_network_simulations.workspace.optimization_projects.CDKL5_DIV21._2_batchrun_optimization.extract_simulated_data import retrieve_sim_data_from_call_stack
...
'''dealbreakers'''
def check_dealbreakers(fitnessVals):
	'''Check if any fitness values are dealbreakers'''
	for key in fitnessVals:
		if key in dealbreakers:
			if fitnessVals[key]['fit'] < dealbreakers[key]:
				return True
	return False
...
def fitnessFunc(simData=None, **kwargs):
    ...
    try: #ensure that the function does not crash - always return a fitness value
        ...
        try:
            ...
            deal_broken = check_dealbreakers(fitnessResults)
