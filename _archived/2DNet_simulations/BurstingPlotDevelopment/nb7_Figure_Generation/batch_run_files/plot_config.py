...
def plot_sim_figs(path, fresh_figs = False, fitness_threshold = None, simLabel = None, net_activity_params = {'binSize': .03*1000, 'gaussianSigma': .12*1000, 'thresholdBurst': 1.0}, timeout = None):
    ## Functions
    def get_fitness_metric():
        try:
            fitness = json.load(open(os.path.join(root, file)))
            average_fitness = fitness['fitness']
            if 'scaled_average_fitness' in fitness: 
                scaled_average_fitness = fitness['average_scaled_fitness']
                #fitness_metric = scaled_average_fitness
                max_fitness_metric = max(average_fitness, scaled_average_fitness)
            else: fitness_metric = average_fitness
        except:
    ...
    output_only = False
    #fresh_figs = False
    #check if path is a filepath isntead of a directory
    file_path = None
    #if path has '/' at the end, remove it
            ...
            if file.endswith('_Fitness.json'):
                ...
                try:
                    ...
                    if fitness_threshold is not None:                        
                        #Get fitness metric
                        fitness_metric = get_fitness_metric()
                        #lower is better, if above threshold, skip
                        if fitness_metric > fitness_threshold: continue               
                    ...
                    if file_path is not None:
                        batchdata_path = file_path
                        simLabel_temp = batchdata_path.split('/')[-1].replace('_data.json', '')
                        assert os.path.exists(batchdata_path), f'{batchdata_path} does not exist'
                        assert '_data.json' in batchdata_path, f'{batchdata_path} is not a _data.json file'
