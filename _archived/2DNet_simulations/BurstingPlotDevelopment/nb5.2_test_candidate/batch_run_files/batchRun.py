...
pops['baseline_diff'] = {'target': 5, 'width': 1.0, 'min': 3}
...
def fitnessFunc(simData, **kwargs):
    ...
    try:
        ...
        try:
            ...
            baseline_diff_fitness = min(np.exp(abs(baseline_diff_target['target'] - baseline_diff) / baseline_diff_target['width']), maxFitness) if baseline_diff > baseline_diff_target['min'] else maxFitness
    ...
    #average fitness
    fitness = (rate_fitness + BurstVal_fitness + IBI_fitness + baselineFitness + slopeFitness + baseline_diff_fitness) / 6

    ##normalized and scaled average fitness - mitigate the effect of outliers
    # Assuming fitness values are stored in a list
    fitness_values = [rate_fitness, BurstVal_fitness, IBI_fitness, baselineFitness, slopeFitness, baseline_diff_fitness]
    ...
    if max_value > min_value:
        ...
        if np.isnan(normalized_fitness_values[0]):
            #set all values to 1
            normalized_fitness_values = [1, 1, 1, 1, 1, 1]
        ...
        scaled_fitness_values = [1000, 1000, 1000, 1000, 1000, 1000] 
