...
nodes=$SLURM_NNODES #hpc, number of nodes
#nodes=1 #if running on laptop or server, number of nodes is 1
...
# njobs=$((SLURM_NNODES))
njobs=2 #until I figure out how to specifically ask for sockets, 1 job per node
gen_pop=4 #one job and one pop per socket

gen_count=0
python batchRun_payload_socket.py -rp ${container_run_path} -d ${Duration_Seconds} -l ${Batch_Run_Label} & export PID=$!
#init
echo '' > commands.txt
