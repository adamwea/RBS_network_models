...
def extract_metrics_from_experimental_data_v3_threads(spike_times, timeVector, spike_times_by_unit, **kwargs):
...
def get_experimental_network_activity_metrics_v2(sorting_object, recording_segment_object, wf_extractor, conv_params, mega_params, **kwargs):
    ...
    try: 
        ...
        extract_metrics_from_experimental_data_v3_threads(spike_times, timeVector, spike_times_by_unit, **kwargs) # aw 2025-02-21 01:12:14 - parallelized version w/ threads might be more efficient?
    except Exception as e:
    ...
    from RBS_network_models.classify_neurons import classify_neurons_v2
    try:
...
def compute_unit_burst_participation(unit_metrics, convolved_data):
    ...
    # Post Process and More Metrics ===============================================================
    # # compute some more metrics
        # for burst_id, burst_part in participation_by_burst.items():
            
        #     #
        #     participation_by_burst[burst_id]['num_units_participating'] = len(burst_part['participating_units'])
            
        #     #
        #     participation_by_burst[burst_id]['spike_counts_by_unit'] = {
        #         'data': burst_part['spike_counts_by_unit'],
        #         'mean': np.nanmean(burst_part['spike_counts_by_unit']),
        #         'std': np.nanstd(burst_part['spike_counts_by_unit']),
        #         'cov': np.nanstd(burst_part['spike_counts_by_unit']) / np.nanmean(burst_part['spike_counts_by_unit']) if np.nanmean(burst_part['spike_counts_by_unit']) > 0 else np.nan
        #     }
            
        #     # calc burst duration using first and last spike time
        #     sorted_spike_times = sorted(list(burst_part['spike_times']))
        #     duration = sorted_spike_times[-1] - sorted_spike_times[0]
        #     participation_by_burst[burst_id]['duration'] = duration
            
        #     # calc spiking rate during burst
        #     participation_by_burst[burst_id]['spike_rate'] = np.sum(burst_part['spike_counts_by_unit']['data']) / duration if duration > 0 else np.nan
            
        #     # calc isi within burst
        #     participation_by_burst[burst_id]['isi'] = {
        #         'data': np.diff(sorted_spike_times),
        #         'mean': np.nanmean(np.diff(sorted_spike_times)),
        #         'std': np.nanstd(np.diff(sorted_spike_times)),
        #         'cov': np.nanstd(np.diff(sorted_spike_times)) / np.nanmean(np.diff(sorted_spike_times)) if np.nanmean(np.diff(sorted_spike_times)) > 0 else np.nan
        #     }
            
        #     # sequence - compose a list of units as they spike chronologically in the burst.
        #     sequence = []
        #     for spike_time in sorted_spike_times:
        #         for unit, spike_times in zip(burst_part['participating_units'], burst_part['spike_times_by_unit']):
        #             if spike_time in spike_times:
        #                 sequence.append(unit)
        #                 break  # Ensure each spike is only counted once            
        #     participation_by_burst[burst_id]['firing_sequence'] = sequence
        #     print(f'Burst {burst_id}: {sequence}')
    
    # compute some more metrics - parallel
    # Parallel execution with a limit of 25 workers
    import os
    max_cpus = os.cpu_count()
    if max_cpus is None:
        max_workers = 1
    if max_cpus <= 25: #HACK: sensitivity to regular laptops
        max_workers = 4
    else:
        max_workers = 25    
...
def compute_burst_metrics(unit_metrics, convolved_data):
    # init
    warnings = None
    ...
    burst_parts = compute_unit_burst_participation(unit_metrics, convolved_data)
    
    #assemble dict
    burst_metrics = {
        'num_bursts': len(peak_times),
        'burst_rate': len(peak_times) / (convolved_data['time_vector'][-1] - convolved_data['time_vector'][0]),
...
def analyze_bursting_activity_v4(spike_times, spike_times_by_unit, min_peak_distance=1.0, binSize=0.001,
    ...
    try:
        ...
        try:
            burst_metrics, burst_warnings = compute_burst_metrics(unit_metrics, convolved_data)
            if burst_warnings:
                print("\n".join(burst_warnings))
        except Exception as e:
