...
def classify(network_data, plot_wfs=False, **kwargs):
    ...
    if plot_clusters:
        ...
        for i, center in enumerate(centroids):
            cluster_label = 'Inhibitory' if i == inhibitory_cluster else 'Excitatory'
            plt.text(center[0], center[1]*1.25, cluster_label, fontsize=12, ha='center', va='center', color='black')
        ...
        plt.close()
        print(f"Saved cluster plot to {os.path.join(wfs_output_path, 'cluster_plot.png')}")
    ...
    for unit in classified_units.keys():
        ...
        # # include x, y coordinates on pc1, pc2 plot
        # classified_units[unit]['pc1'] = features_pca[i, 0]
        # classified_units[unit]['pc2'] = features_pca[i, 1]
    ...
    return classified_units, include_units, exclude_units, features_pca
            
def classify_neurons_v2(network_data, plot_wfs=False, **kwargs):
    
    classified_units, include_units, exclude_units, features_pca = classify(network_data, plot_wfs=plot_wfs, **kwargs)
