...
# bash srun_payload_int.sh

###OPTIONS
Duration_Seconds=90 #Simulation Duration in seconds
Batch_Run_Label='this_does_work' #Label for the batch run folder
...
python batchRun_payload.py \
    -rp ${run_path} \
    -d ${Duration_Seconds} \
    -l ${Batch_Run_Label} \
    & export PID=$! #run batchRun_payload.py in the background
#check for folders named gen_# in run_path
#get the number of the latest generation
gen_count=$(ls -l ${run_path} | grep -c -P '^d.*gen_\d+$')
...
#bash wait_for_current_gen.sh $current_gen
bash wait_for_commands.sh $gen_pop #wait for batchRun_*.py to finish sending the next srun commands to commands.txt
