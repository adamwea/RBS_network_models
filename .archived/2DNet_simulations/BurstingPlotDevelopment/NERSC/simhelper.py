'''
The file simhelper.py is a collection of useful functions written for batch processing of HD-MEA simulations using NetPyNE. It includes functions for loading SVG paths, retrieving batch run information, and measuring network activity. The file is adapted from a previous version and the author list is not provided.
Authors: Adam W, Mandar MP
...
def generate_all_figures(fresh_figs = False, batchLabel = 'batch', batch_path = None, size = 6, net_activity_params = {'binSize': 3, 'gaussianSigma': 12, 'thresholdBurst': 1.0}):
    ...
    def generate_param_summary_row(file_path, TwoDNet_path, conn_mat_path, soma_voltage_trace_path, raster_plot_path, net_activity_plot_path, output_folder, output_key):
        param_summary_row_path = f'{output_folder}/{output_key}_param_summary_row.svg'
        try: 
            # Load the SVG files
            TwoDNet_plot = sg.fromfile(TwoDNet_path)
            conn_mat = sg.fromfile(conn_mat_path)
            soma_voltage_trace = sg.fromfile(soma_voltage_trace_path)
            raster_plot = sg.fromfile(raster_plot_path)
            net_activity_plot = sg.fromfile(net_activity_plot_path)
            ...
            # Load params.json
            grandparent_folder = os.path.dirname(os.path.dirname(output_folder))    
            ...
            dwg = Drawing('background.svg', profile='tiny')
            ...
            background = sg.fromfile("background.svg").getroot()
            ...
            for i, key in enumerate(params.keys()):
                ...
                plot = sg.fromfile("temp_plot.svg").getroot()
            ...
            print(f'Error generating param_summary_row figure at: {output_folder}')
            return None
