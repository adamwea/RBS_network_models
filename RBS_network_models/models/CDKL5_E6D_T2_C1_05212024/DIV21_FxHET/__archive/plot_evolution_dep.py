...
def group_by_generation(fitness_entries):
    ...
    # only show the top 256 in each generation
    # use np sortidx to get the indices of the top 256
    # for gen in grouped:
    #     if len(grouped[gen]) > 128:
    #         top_indices = np.argsort(grouped[gen])[:128]
    #         grouped[gen] = [grouped[gen][i] for i in top_indices]
...
def plot_fitness_boxplots(grouped_fits, output_dir):
    ...
    svg_path = os.path.join(output_dir, "fitness_evolution_plot.svg")
...
if __name__ == "__main__":
    ...
    output_dir = batch_paths[0] + '/evolution_reports/'
