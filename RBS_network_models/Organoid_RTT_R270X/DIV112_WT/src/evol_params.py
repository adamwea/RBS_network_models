...
if version == 2.0:
    ...
    params['propVelocity'] = [0.1, 1.0]  # Conduction velocities in mammalian neurons range from ~0.1 m/s in unmyelinated fibers to over 100 m/s in large myelinated axons. [Source: Wikipedia - Axon](https://en.wikipedia.org/wiki/Axon)
    # NOTE:
        # Propagation Parameters
    ...
    params.update({
        ...
        'E_L_mean': [50, 1200],  # Mean length of excitatory neurons (µm). Individual dendrites are usually several hundred micrometers long. [Source: Pyramidal cell](https://en.wikipedia.org/wiki/Pyramidal_cell)
        'E_Ra_mean': [50, 200],  # Mean axial resistance of excitatory neurons (Ω·cm). Axial resistance is inversely related to the cross-sectional area of the neuron. [Source: Length constant](https://en.wikipedia.org/wiki/Length_constant)

        'I_diam_mean': [4, 18],  # Mean diameter of inhibitory neurons (µm). Inhibitory interneurons have soma diameters averaging around 10 µm, with some variability. [Source: Golgi cell](https://en.wikipedia.org/wiki/Golgi_cell)
        'I_L_mean': [50, 600],  # Mean length of inhibitory neurons (µm). Inhibitory neurons typically have shorter dendrites compared to excitatory neurons.
