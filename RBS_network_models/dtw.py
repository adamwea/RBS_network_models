...
def dtw_burst_analysis(network_metrics, **kwargs):
    ...
    if not os.path.exists(mega_data_path):
        os.makedirs(mega_data_path)
        
    # save network metrics dirs
    network_metrics['dtw_output'] = data_path
    network_metrics['mega_dtw_output'] = mega_data_path
...
def dtw_analysis_dynamic_v2(sequence_stacks, data_path=None, 
                            cv_threshold=0.05, moving_avg_window=500, moving_avg_threshold=0.01, confidence_threshold=0.99,
    ...
    #num_workers = min(multiprocessing.cpu_count(), 50)  # Limit to 50 workers
    # batch_size = 1000
