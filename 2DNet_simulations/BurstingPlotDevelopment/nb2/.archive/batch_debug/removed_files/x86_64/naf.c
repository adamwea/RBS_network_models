...
#define _f_settables _f_settables__naf 
#define settables settables__naf 
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 naf /mnt/disk15tb/adam/git_workspace/netpyne/netpyne/tutorials/mod/naf.mod\n");
...
/*CVODE*/
 ...
 static int settables(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _lv1) { 
#if 0
_check_settables(_p, _ppvar, _thread, _nt);
#endif
 ...
 static void _n_settables(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _lv1){ int _i, _j;
 double _xi, _theta;
 if (!usetable) {
...
static int  _f_settables ( _threadargsprotocomma_ double _lv1 ) {
   ...
   if ( ( _lv1 + fastNashift ) < - 30.0 ) {
     mtau = 0.025 + 0.14 * exp ( ( ( _lv1 + fastNashift ) + 30.0 ) / 10.0 ) ;
     }
   else {
     mtau = 0.02 + 0.145 * exp ( ( - ( _lv1 + fastNashift ) - 30.0 ) / 10.0 ) ;
     }
   hinf = 1.0 / ( 1.0 + exp ( ( ( _lv1 + fastNashift * 0.0 ) + 62.9 ) / 10.7 ) ) ;
   htau = 0.15 + 1.15 / ( 1.0 + exp ( ( ( _lv1 + fastNashift * 0.0 ) + 37.0 ) / 15.0 ) ) ;
    return 0; }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/mnt/disk15tb/adam/git_workspace/netpyne/netpyne/tutorials/mod/naf.mod";
static const char* nmodl_file_text = 
