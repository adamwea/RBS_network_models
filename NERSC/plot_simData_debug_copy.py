...
def generate_pdf_page(data_file_path, elite_paths_cull, gen_rank, HOF_path=None):
    ...
    if '.archive' in data_file_path:
        ...
        load_path = data_file_path.replace('/output/', '/plots/')
    ...
    if not HOF_mode: conns_plot_path = os.path.join(cand_plot_path, f'{simLabel}_connections.png')
    else: conns_plot_path = os.path.join(load_path, f'{simLabel}_connections.png')
