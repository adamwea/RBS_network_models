...
ntasks=4 #need to find a nuumber of tasks that works well. Ideally some multiple of pop.
...
# export OMPI_MCA_btl_sm_exclude="1"
# export OMPI_MCA_btl_base_verbose="100"
# export OMPI_MCA_pml_ob1_max_send_requests="256"
...
#srun --mcva btl ^sm --verbose -np ${ntasks} -x TMPDIR=$TMPDIR \
mpiexec --mca pml ob1 --mca btl tcp,self --mca btl_tcp_if_include eth1 -np ${ntasks} -x TMPDIR=$TMPDIR \
nrniv -mpi -python batchRun_mpi.py -rp ${container_run_path} -d ${Duration_Seconds} -l ${Batch_Run_Label}

#srun --mca btl ^sm --verbose -np ${ntasks} -x TMPDIR=$TMPDIR \
#nrniv -mpi -python batchRun_mpi.py -rp ${container_run_path} -d ${Duration_Seconds} -l ${Batch_Run_Label}
